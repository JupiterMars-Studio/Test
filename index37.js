...
//   const iconWidth = 12.47;
//   const gap = 18;
//   const navWidth = document.querySelector('.navbar').offsetWidth;
//   return Math.floor((navWidth + gap) / (iconWidth + gap));
// }
...
//   // optionally re-render icons here
// });


  // const totalCount = isMobile ? 18 : 40;
...
//    if (i === totalCount - 2) {
//     clone.classList.add("second-last");
//     clone.classList.remove("show");
//   }
...
// }
  ...
  const moveDistance = wrapperWidth;

  gsap.set(".bracket-text", { opacity: 0 });
  // gsap.set(".section_home-header", { y: "100vh" });
  gsap.set(".section_home-header", { y: 100, autoAlpha: 0 });
  ...
  gsap.set(".heading-hero", {y: 0, opacity: 1})
  gsap.set(".hero-description", {y: 0, opacity: 1})
  gsap.set(".button-hero_wrapper", {y: 0, opacity: 1})
  gsap.set(".masking-left", { width: "120px", display: "block" });
  gsap.set(".masking-right", { width: "333px", display: "block" });
  gsap.set(".last", { visibility: "hidden" });

  const navbarWidth = document.querySelector(".navbar-container").offsetWidth;

const tl = gsap.timeline()
  // .from(".show", {visibility: "hidden", stagger: 0.03, ease: "none"})
  // .from(".second-last", {visibility: "hidden", ease: "none", duration: 0.02})
  // .from(".last", {visibility: "hidden", ease: "none"})
  // .to(".last", {opacity: 1, x: -moveDistance, ease: "none", duration: isMobile ? 0.2 : 0.34, delay: 1}, "<")
//   .to(".show", {
//   visibility: "hidden",
//   stagger: {
//     each: 0.03,
//     from: "end" // ensures first to last
//   },
//   ease: "none"
// }, "<")

  .to(".section_home-header", { y: 0, duration: 0.2, autoAlpha: 1, ease: "none"})
  .to(".section_scrolling-logo", {y: 0, duration: 0.4, autoAlpha: 1, ease: "power2.out"}, "<")
  .to(".bracket-end-nav", {display: "flex", duration: 0.5, ease: "none"}, "<")
  .to(".navbar", {opacity: 1, duration: 0.5, ease: "none"})
  .to(".link-hover-line", {opacity: 1, duration: 0.2, ease: "none"}, "<")
  ...
  .from(".flowappz-cookie-consent-popup-14", {autoAlpha: 0, delay: 8})

} else{
      mm.add("(min-width: 767.5px)", () => {
        gsap.to(".heading-hero", { y: 0, duration: 0.4, autoAlpha: 1, ease: "power1.in" });
        gsap.to(".hero-description", { y: 0, duration: 0.4, autoAlpha: 1, ease: "power1.in" }, "<");
        gsap.to(".button-hero_wrapper", { y: 0, duration: 0.5, autoAlpha: 1, ease: "power1.in" }, "<");
        // gsap.to(".section_scrolling-logo", { y: 0, duration: 0.1, ease: "power1.in" });
        gsap.to(".navbar", { duration: 0.2, autoAlpha: 1, ease: "power1.in" });
        gsap.to(".hero-grid", { opacity: 1, duration: 0.2, ease: "power1.in" });
        gsap.set("body", {overflow: "auto"}, "<")
        setTimeout(runAutoExpandIcons, 600);
        gsap.set(".loader", { display: "none" });
        gsap.from(".flowappz-cookie-consent-popup-14", {autoAlpha: 0, delay: 8});
  });
}
...
const specialMobileIndexes = [262, 270, 286, 318];
const specialColor = [
  "quaternary-color",
  "plasma-fluor",
  "laser-fluor",
...
const textsEs = [
  "Diseño B2B",                 
  "Diseño de Marketing B2B",    
  "Operaciones de Diseño B2B",  
  "Branding B2B",               
];
...
const mql = window.matchMedia("(max-width: 767.5px)");
  ...
  function buildGrid() {
    ...
    const rows = mobile ? 24 : 18;
    const columns = mobile ? 14 : 30;
    const activeSpecialIndexes = mobile ? specialMobileIndexes : specialIndexes;

    // Clear previous nodes
    heroGrid.replaceChildren();

    // Build fast using a fragment
    ...
    for (let i = 0; i < rows * columns; i++) {
      ...
      const labelIndex = isSpecial ? activeSpecialIndexes.indexOf(i) : -1;
  ...
  // Rebuild when crossing the breakpoint (mobile <-> desktop)
  mql.addEventListener("change", buildGrid);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    mm.add("(min-width: 767.5px)", () => {
      ...
      allLis.forEach((li, index) => {
        ...
        if (!isLast) {
          tlHows.to(li, { opacity: 0.65, x: -100 }, index * 0.5);
        }
      });
    ...
    mm.add("(max-width: 767.5px)", () => {
      ...
      allLis.forEach((li, index) => {
        ...
        // Animate out (skip last one)
        if (!isLast) { 
          tlHows.to( li, { opacity: 0.65, x: -10}, index * 0.5 );
        }
      });
...
const panels = gsap.utils.toArray(".what-we_panel");
  ...
  const master = gsap.timeline({
      scrollTrigger: {
        ...
        markers: false,
        start: "top top",
        end: () => "+=" + window.innerWidth * panels.length, // extra scroll space
      },
    });
...
mm.add("(min-width: 767.5px)", () => {
  ...
  gsap.set(".connect_logo", {opacity: 1});
  ...
  postScrollLogoAnim
    ...
    .to(".connect_logo", { top: "82.2px",  yPercent: 0, ease: "none", duration: 1.5 }, "<+1")
