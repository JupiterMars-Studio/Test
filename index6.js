...
mm.add("(min-width: 769px)", () => {
  const BASE_SCROLL_SPEED = 1000;

  // Calculate dynamic scroll duration based on .scrollWidth - .offsetWidth
...
let totalOffset = 0;
  ...
  panels.forEach((panel) => {
    ...
    const scrollDistance =
      inner && inner.scrollWidth > panelWidth
        ? inner.scrollWidth - panelWidth
        : 0;

    const scrollUnits = scrollDistance > 0 ? scrollDistance / BASE_SCROLL_SPEED : 0.3;
    totalScrollUnits += scrollUnits + 0.5; // +0.5 for panel slide

  panelOffsets.push(totalOffset);
  totalOffset += panelWidth;
  });

  const totalScrollLength = window.innerHeight * totalScrollUnits;
  ...
  panels.forEach((panel, i) => {
    ...
    // Step 3: Animate items inside each panel
